...
class VeiculoPasseioControllerTest {
    ...
    void addVeiculoPasseioValido() throws Exception {
        VeiculoPasseioDTO veiculoPasseio = new VeiculoPasseioDTO();
        ...
        when(veiculoPasseioService.cadastraVeiculoPasseio(any(VeiculoPasseioDTO.class))).thenReturn(veiculoPasseio);

        mockMvc.perform(post("/veiculos/passeio/add").contentType(MediaType.APPLICATION_JSON).content("{ \"nome\": \"Corolla\" }")).andExpect(status().isCreated()).andExpect(jsonPath("$.id").value(1)).andExpect(jsonPath("$.nome").value("Corolla")).andExpect(jsonPath("$.marca").value("Toyota")).andExpect(jsonPath("$.placa").value("ABCD1234")).andExpect(jsonPath("$.numeroDePassageiros").value(5));
    }
    ...
    void getVeiculoPasseioById() throws Exception {
        ...
        mockMvc.perform(get("/veiculos/passeio/{id}", 1)).andExpect(status().isOk()).andExpect(jsonPath("$.id").value(1)).andExpect(jsonPath("$.nome").value("Corolla"));
    }
    ...
    void atualizarVeiculoPasseio() throws Exception {
        ...
        mockMvc.perform(put("/veiculos/passeio/update/{id}", 1).contentType(MediaType.APPLICATION_JSON).content("{ \"nome\": \"Fusca\" }")).andExpect(status().isOk()).andExpect(jsonPath("$.id").value(1)).andExpect(jsonPath("$.nome").value("Fusca"));
    }

    @Test
    void deleteVeiculoPasseio() throws Exception {
        mockMvc.perform(delete("/veiculos/passeio/delete/{id}", 1)).andExpect(status().isNoContent());
